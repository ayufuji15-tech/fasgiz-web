...
function fill(){
  ...
  $('insulinRows').innerHTML=['Pagi','Snack Pagi','Siang','Snack Sore','Malam'].map((w,i)=>`
    <tr>
      ...
      <td><input id="insRasio${i}" type="number" step="0.1" value="15"></td>
...
function diff(a,b=new Date()){
  ...
  return{
    ...
    d,
    months:y*12+m,
    days:Math.max(0,Math.floor((b-a)/86400000))
  };
}

function usiaText(x){
  return x?`${x.y} th ${x.m} bln ${x.d} hr`:'-';
}
...
function harris(jk,bb,tb,u){
  return jk==='Laki-laki'
    ...
    :655.1+9.563*bb+1.85*tb-4.676*u;
}

function macro(tee,pGram,lPct){
  const l=tee*lPct/100/9;
  const k=(tee-pGram*4-l*9)/4;
...
function row(a,b,c=''){
  return `<tr><td>${a}</td><td><b>${b}</b></td><td>${c}</td></tr>`;
}
...
function hitungAntro(){
  ...
  $('antroRows').innerHTML=[
    ...
    row(
      ...
      interpretImtDewasa(imtAktual),
      ''
    ),

    row(
      '% LILA/U',
      pctLila?satu(pctLila):'0,0',
      '%'
    ),
    ...
    row(
      'IMT Estimasi dari LILA',
      imtLila?satu(imtLila):'0,0',
      'kg/m²'
    ),
    ...
    row(
      'BB estimasi dari LILA Paravista',
      satu(l?l*2.5-5:0),
      'kg'
    ),

    row(
      'BB estimasi dari LILA Cerra',
      satu(l?l*2.2+7:0),
      'kg'
    ),

    row(
      'TB estimasi dari Ulna Ilayperuma',
      satu(
        ul
          ?(
            jk==='Laki-laki'
    ...
    row(
      'BB estimasi dari lingkar betis',
      satu(
        bet
          ?bet*1.5+20
          :0
...
function hitungAnak(){
  ...
  function r(n,rg,v){
    return `<tr><td>${n}</td><td>${rg}</td><td>${v?satu(v):'di luar rentang'}</td></tr>`;
  }

  $('anakRuleRows').innerHTML=[
    r('Berat Badan','0–12 bulan',months<=12?months/2+4:0),
    r('Berat Badan','1–6 tahun',years>=1&&years<=6?years*2+8:0),
    r('Berat Badan','7–12 tahun',years>=7&&years<=12?years*3+7:0),
    r('Tinggi Badan','0–2 tahun',months<=24?months*2.5+50:0),
    r('Tinggi Badan','2–12 tahun',years>=2&&years<=12?years*6+77:0),
    r('Lingkar Kepala','0–12 bulan',months<=12?35+months:0),
    r('Lingkar Kepala','>1 tahun',years>=1?46+years*0.5:0)
  ].join('');
...
function hitungNeed(){
  ...
  const methods=[
    ['mif','Mifflin-St Jeor',mifflin(jk,bb,tb,u)],
    ['hb','Harris-Benedict',harris(jk,bb,tb,u)],
    ['fast','Rumus Cepat',bbi*num('fastKkal')]
  ];

  $('ndRows').innerHTML=
    methods.map(([id,name,bmr])=>{
      ...
      return `
        <tr>
          ...
          <td>${satu(l)}</td>
          <td>${satu(k)}</td>
          <td><button onclick="useNeed('nd:${id}')">Gunakan</button></td>
        </tr>
      `;
    }).join('');
...
function hitungAnakNeeds(){
  ...
  $('rdaRows').innerHTML=
    `<tr>
      <td>${satu(num('rdaBb')*num('rdaUsia'))}</td>
      <td>${satu(num('rdaBb')*num('rdaProtKg'))}</td>
    </tr>`;
  ...
  $('ventRows').innerHTML=
    `<tr>
      <td>${satu(num('ventBb')*lo)}–${satu(num('ventBb')*hi)}</td>
      <td>${satu(num('ventBb')*num('ventProtKg'))}</td>
    </tr>`;
}
...
function hitungDm(){
  ...
  for(let i=0;i<5;i++){
    ...
    $('insUnit'+i).textContent=
      satu(u);
  }

  $('insKhTotal').textContent=
    satu(tkh);

  $('insUnitTotal').textContent=
    satu(tu);
}
...
function calcPick(list,selId,qId,outIds,per100=false){
  ...
  $(outIds[0]).textContent=
    satu(o.e);

  $(outIds[1]).textContent=
    satu(o.p);

  $(outIds[2]).textContent=
    satu(o.l);

  $(outIds[3]).textContent=
    satu(o.k);
...
function hitungAsupan(){
  ...
  for(let i=0;i<4;i++){
    ...
    $(`recallE${i}`).textContent=
      satu(o.e);

    $(`recallP${i}`).textContent=
      satu(o.p);

    $(`recallL${i}`).textContent=
      satu(o.l);

    $(`recallK${i}`).textContent=
      satu(o.k);
  }
...
function hitungMonitoring(){
  ...
  const pct={
    e:need.e?all.e/need.e*100:0,
    p:need.p?all.p/need.p*100:0,
    l:need.l?all.l/need.l*100:0,
    k:need.k?all.k/need.k*100:0
  };
  ...
  $('monitorRows').innerHTML=[
    tr('Pemenuhan Diet Parenteral',total.pn),
    tr('Pemenuhan Diet Oral',oral),
    tr('Pemenuhan Diet Enteral',enteral),
    tr('<b>Total Pemenuhan</b>',all),
    tr(`<b>Total Kebutuhan Pasien</b><br><small>${need.source}</small>`,need),
    tr('<b>% Pemenuhan</b>',pct)
  ].join('');
}
